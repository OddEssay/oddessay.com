...
export default function Shell({ section, children }: { section?: string; children: ReactNode }) {
  return <div className="site-shell"><SketchAppShell
    ...
    sidebar={<nav className="sidebar" aria-label="Main navigation"><p className="eyebrow">Contents</p>{['Projects', 'Restaurants', 'Essays'].map((label, index) => <a key={label} href={`/${label.toLowerCase()}`} aria-current={section === label.toLowerCase() ? 'page' : undefined}><span className="nav-number">0{index + 1}</span>{label}<span aria-hidden="true">↗</span></a>)}<p className="sidebar-note">Here be <s>Dragons</s>Cheese</p></nav>}
